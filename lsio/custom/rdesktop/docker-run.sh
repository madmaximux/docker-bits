...
docker run -d \
  ...
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/rdesktop/config:/config `# Configuration files.` \
  ...
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/rdesktop/config:/config `# optional` `# abc users home directory` \
