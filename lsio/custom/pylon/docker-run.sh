...
docker run -d \
  ...
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/pylon/config:/config `# Configuration files.` \
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/pylon/code:/code `# optional` `# Optionally if you want the bind mount your own code and have changes survive container upgrades.` \
