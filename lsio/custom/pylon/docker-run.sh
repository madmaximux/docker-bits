...
docker run -d \
  ...
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/pylon${DOCKERCONFIGDIR:-}:/config \
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/pylon/code:/code `# Optionally if you want the bind mount your own code and have changes survive container upgrades.` \
