...
docker run -d \
  ...
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/sickgear/tv:/tv `# where you store your tv shows` \
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/sickgear/downloads:/downloads `# your downloads folder for post processing (must not be download in progress)` \
