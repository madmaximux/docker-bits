...
docker run -d \
  ...
  -p 443:443 `# WebUI` \
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/nextcloud/config:/config `# Persistent config files` \
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/nextcloud/data:/data `# Your personal data.` \
