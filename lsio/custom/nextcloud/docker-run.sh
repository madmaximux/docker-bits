...
docker run -d \
  ...
  -p 443:443 `# WebUI` \
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/nextcloud/config:/config `# Persistent config files` \
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/nextcloud/data:/data `# Your personal data.` \
