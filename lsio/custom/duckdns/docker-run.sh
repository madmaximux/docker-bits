...
docker run -d \
  ...
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/duckdns/config:/config `# Configuration files.` \
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/duckdns/config:/config `# optional` `# Persistent config files. Also set `LOG_FILE=true` to keep address history.` \
