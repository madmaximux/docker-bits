...
docker run -d \
  ...
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/pyload-ng/config:/config `# Persistent config files` \
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/pyload-ng/downloads:/downloads `# Destination of pyLoad downloads` \
