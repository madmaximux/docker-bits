...
docker run -d \
  ...
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/flexget/config:/config `# Local path for FlexGet config files.` \
  -v ${DOCKERCONFIGDIR:-/volume1/docker/appdata}/flexget/data:/data `# optional` `# Default path for downloads.` \
