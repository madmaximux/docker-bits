...
docker run -d \
  ...
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/netbootxyz${DOCKERCONFIGDIR:-}:/config \
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/netbootxyz/assets:/assets `# Storage for NETBOOT.XYZ bootable assets (live CDs and other files)` \
