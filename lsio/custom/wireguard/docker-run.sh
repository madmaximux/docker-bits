...
docker run -d \
  ...
  -v ${DOCKERCONFIGPATH:-/volume1/docker/appdata}/wireguard${DOCKERCONFIGDIR:-}:/config \
      # Path to host kernel module for situations where it's not already loaded.
  -v /lib/modules:/lib/modules  `# Path to host kernel module for situations where it's not already loaded.` \
